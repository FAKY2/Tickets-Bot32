...
bot.on("ready", async () => {
    ...
    await bot.editStatus("online", { type: 0, name: process.env["status"] || "By FAKY" })
...
bot.on("interactionCreate", async (interaction) => {
    if (interaction.type == 5) {
        if (interaction.data.custom_id.startsWith("ct")) {
            ...
            await interaction.createMessage({
                embeds: [{
                    description: `☑️ Ticket locked by <@!${interaction.member.id}>`,
                    fields: [{ name: "resone", value: `\`\`\`${interaction.data.components[0].components[0].value}\`\`\`` }],
                    color: 0xdfb21f
                }]
            }).catch(() => { });
            ...
            setTimeout(async () => {
                await bot.createMessage(interaction.data.custom_id.split("_")[4], {
                    embeds: [{
                        ...
                        description: `The ticket has been deleted by <@!${interaction.member.id}>`,
                        fields: [{ name: "ticket", value: `<#${interaction.data.custom_id.split("_")[2]}>` }, { name: "resone", value: `\`\`\`${interaction.data.components[0].components[0].value}\`\`\`` }, { name: "ticket type", value: `\`\`\`${interaction.data.custom_id.split("_")[3].replaceAll("-", " ")}\`\`\`` }, { name: "ticket by ", value: `<@!${interaction.data.custom_id.split("_")[1]}>` }],
                        color: 0xdfb21f,
                        timestamp: new Date()
                    }]
                }, {
    ...
    if (interaction.type == 3) {
        if (interaction.data.custom_id.startsWith("ct_")) {
            if (!await user_tickets_db.has(`${interaction.data.custom_id.split("_")[1]}`)) return await interaction.createMessage({
                flags: 64,
                content: ":x: There is no information about these buttons !"
            })
            if (interaction.channel.name.startsWith("closed")) return await interaction.createMessage({
                flags: 64,
                content: ":x: This ticket is already closed."
            ...
            if (interaction.data.custom_id.split("_")[1] != interaction.member.id && !interaction.member.roles.some(x => x == interaction.data.custom_id.split("_")[5])) return await interaction.createMessage({
                flags: 64,
                content: "Only the ticket holder or the lock crew can ticket from this button :x:"
            })
            await bot.createInteractionResponse(interaction.id, interaction.token, {
                type: 9, data: {
                    "title": "Exploring the reason for locking the ticket",
                    ...
                    "components": [
                        {
                            ...
                            "components": [
                                {
                                    "type": 4,
                                    "label": "Write the reason for delete the ticket",
                                    "style": 2,
                                    "custom_id": "reason",
                                    "placeholder": "Write the reason for delete the ticket",
                                    "required": true,
                                    "min_length": 1,
                                    "max_length": 250
                                }
        ...
        if (interaction.data.custom_id.startsWith("tt_")) {
            ...
            if (!interaction.member.roles.some(x => x == interaction.data.custom_id.split("_")[5])) return await interaction.createMessage({
                flags: 64,
                content: "Only technical support staff can do this procedure  :x:" + `\n<@&${interaction.data.custom_id.split("_")[5]}>`
            })
            await interaction.defer()
            let msgs = await bot.getMessages(interaction.channel.id, { limit: 500 })
            await interaction.createFollowup({
                embeds: [{
                    title: "Ticket transferred to conversation",
                    description: `The ticket was transferred to the conversation by  <@!${interaction.member.id}>`,
                    color: 0xdfb21f,
                    timestamp: new Date()
                }]
            }, {
            ...
            setTimeout(async () => {
                ...
                await bot.createMessage(interaction.data.custom_id.split("_")[4], {
                    embeds: [{
                        ...
                        fields: [{ name: "Ticket", value: `${interaction.data.custom_id.split("_")[2]}` }, { name: "نوع التذاكر", value: `\`\`\`${interaction.data.custom_id.split("_")[3].replaceAll("-", " ")}\`\`\`` }, { name: "ticket by", value: `<@!${interaction.data.custom_id.split("_")[1]}>` }],
                        color: 0xdf1f1f,
                        timestamp: new Date()
                    }]
                }, {
        ...
        if (interaction.data.custom_id.startsWith("ot")) {
            ...
            if (!interaction.member.roles.some(x => x == interaction.data.custom_id.split("_")[5])) return await interaction.createMessage({
                flags: 64,
                content: " Only technical support staff can do this :x:" + `\n<@&${interaction.data.custom_id.split("_")[5]}>`
            })
            await bot.deleteMessage(interaction.channel.id, interaction.message.id)
            await bot.createMessage(interaction.channel.id, {
                embeds: [{
                    description: `☑️  Ticket opened by ☑️ <@!${interaction.member.id}>`,
                    color: 0x3FB745
                }]
            }).catch(() => { });
            let ticket_by = cd_data.split("_")[1]
            ...
            await bot.createMessage(interaction.data.custom_id.split("_")[4], {
                embeds: [{
                    ...
                    description: `The ticket was transferred to the conversation by <@!${interaction.member.id}>`,
                    fields: [{ name: "ticket", value: `<#${interaction.data.custom_id.split("_")[2]}>` }, { name: "ticket type", value: `\`\`\`${interaction.data.custom_id.split("_")[3].replaceAll("-", " ")}\`\`\`` }, { name: "ticket type", value: `<@!${interaction.data.custom_id.split("_")[1]}>` }],
                    color: 0x3FB745,
                    timestamp: new Date()
                }]
            })
        ...
        if (interaction.data.custom_id.startsWith("ticket")) {
            ...
            if (!await tickets_db.has(`${ticket_id}`)) return await interaction.createMessage({
                flags: 64,
                content: "This class of tickets is no longer operational."
            })
            ...
            bot.createChannel(interaction.guildID, `ticket-${padNum(count + 1, 4)}`, 0, {
                ...
                await bot.createMessage(interaction.data.custom_id.split("_")[4], {
                    embeds: [{
                        ...
                        description: `Your ticket has been created <@!${interaction.member.id}>`,
                ...
                await channel.createMessage({
                    content: `hey , <@!${interaction.member.id}>, <@&${interaction.data.custom_id.split("_")[2]}>`, embeds: [
                        {
                            title: `${interaction.data.custom_id.split("_")[1].replaceAll("-", " ")}`,
                            description: `، Thanks for creating the ticket. Please wait for the support team. They will respond to you. If you want to lock the ticket, press the button 🔒${interaction.data.custom_id.split("_")[5] == true ? "" : `\n\`\`\`${interaction.data.custom_id.split("_")[5].replaceAll("-", " ")}\`\`\` `}`,
                            color: 0x3FB745,
                            footer: { text: "Please abide by the laws" }
                        }
                    ],
                    components: [{
                        type: 1, components: [{
    ...
    if (interaction.type == 2) {
        if (interaction.data.name == "setup") {
            if (!interaction.member.permissions.has("administrator")) return await interaction.createMessage({
                flags: 64,
                content: ":x: You don't have powers for this."
            })
            ...
            await interaction.createMessage({
                flags: 64,
                embeds: [{
                    title: "☑️ Your request has been successfully processed.",
                    color: 0x3FB745
                }]
            });
            let components = [];
